...
int main(){
	...
	if (emaitza == 1){
		printf("Eskolako datuak ondo irakurri ditu\n");
	}
...
int fitxeroanIrakurri(ESKOLA eskolak[ESKOLAMAX]){
	...
	//programa
	if ((fitxeroa = fopen("data.txt", "rb")) == NULL){
		egokia = 0;
	}
	else{
		karaktereKopurua = fread(eskolak, sizeof(ESKOLA), ESKOLAMAX, fitxeroa);
		egokia = 1;
	}
	//return 
	return egokia;
...
int fitxeroanIdatzi(ESKOLA eskolak[ESKOLAMAX]){
	...
	//programa
	if ((fitxeroa = fopen("data.txt", "wb")) == NULL){
		printf("Ezin izan da datuak gordetzeko fitxeroa aurkitu\n");
		egokia = 0;
	}
	else{
		karaktereKopurua = fwrite(eskolak, sizeof(ESKOLA), ESKOLAMAX, fitxeroa);
		egokia = 1;
	}
